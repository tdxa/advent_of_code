...
export const recoverCalibrationValues = (
  lines: Array<string>
): Array<string> => {
  const linesDigits = lines.map((line) => line.match(/\d/g)) as Array<
    Array<string>
  >;
  const calibrationValues = linesDigits.map(
...
const calibrationSum = sumArray(recoverCalibrationValues(replacedWords));
